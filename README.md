...
局域网多跳 Mesh 通信软件。同一群组内的设备只要存在任意直连链路即可通信，消息可经中间设备多跳转发到达目标，不要求全网互相连通。支持群聊、单聊、文件传输与更新互助，无需互联网与中心服务器。
...
- **成员状态与昵称**：实时展示成员上线/下线、在线状态，以及直连与多跳可达标记；设置的昵称会广播给群组其他成员，无需对方手动备注。
- **连接保活**：邻居间定时心跳，超时自动判定离线；Leaf 角色断线后指数退避自动重连。
- **多网卡选择**：创建或加入群组时可指定本机网卡，避免系统默认路由选错网络接口；加入群组时的中继发现在所有本机网卡上并行扫描，多网卡环境下发现更快速、更可靠。
- **一键分享码快速加群**：建群方可将群组名、Group ID 与各网卡可连接地址压缩为一段分享码；新设备粘贴分享码即可自动填入加群信息，并自动探测当前可用的中继地址与本机网卡，无需手动逐项输入。
- **客户端自动更新**：程序启动时自动检查新版本，也可手动触发检查；发现新版本后可查看版本号、发布时间与更新说明，确认后自动下载并安装，无需手动下载安装包。
- **更新互助**：下载的更新安装包可分享给局域网群组中的其他设备；接收方完成完整性校验后可一键确认安装，让无法访问更新服务器的受限网络设备也能离线升级。
- **托盘驻留与桌面通知**：关闭或最小化窗口时驻留系统托盘继续运行，可通过托盘菜单打开主界面或退出；窗口隐藏期间收到的新消息会弹出 Windows 桌面通知，点击通知直接回到主界面。两项行为均可在配置页开关。
...
核心库包含会话隔离、消息去重、路径发现等单元测试，桌面端 Rust 后端另有版本比较、事件序列化等少量单元测试：
